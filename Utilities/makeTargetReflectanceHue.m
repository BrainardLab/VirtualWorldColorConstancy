function makeTargetReflectanceHue(HueLevels,reflectanceNumbers, folderToStore)
% makeTargetReflectanceHue(HueLevels, reflectanceNumbers, folderToStore)
%
% Usage: 
%     makeTargetReflectanceHue([0.1 0.1 1], [1:5], 'ExampleFolderName')
%
% Description:
...
newSurfaces = zeros(S(3),size(HueLevels,2)*nsurfacePerXYZ);
newIndex =0;
...
while (m < nsurfacePerXYZ)
    randomSaturations = satMin + (satMax - satMin)*rand();
    randomLigthness = lightMin + (lightMax - lightMin)*rand();

    XYZLevel = rgb2xyz(hsv2rgb([HueLevels(ii) randomSaturations randomLigthness]));
    ...
    reflectanceName = sprintf('luminance-%.4f-reflectance-%03d.spd', HueLevels(ii), ...
