...
%
% recipePath = AwsRecipeForCondition(luminanceLevel, reflectanceNumber)
% searches our S3 bucket mount for the recipe that was run with the given
% luminance level and reflectance number.
%
% AwsRecipeForCondition( ... 'bucketFolder', bucketFolder) specify the
...
subFolderPath = fullfile(bucketFolder, jobFolder, 'VirtualWorldColorConstancy', 'ToyVirtualWorld', subfolder);
...
% we divided up jobs 5 reflectances at a time
reflectancesPerJob = 5;
jobNumber = ceil(reflectanceNumber / reflectancesPerJob);

% we had an almost-consistent naming convention for jobs
if jobNumber < 11
    jobFolder = sprintf('job-%d-no-return', jobNumber);
else
